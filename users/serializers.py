...
from django.contrib.auth import authenticate # Django의 기본 authenticate 함수 -> TokenAuth 방식으로 유저 인증.
...
class Meta:
    ...
    def create(self, validated_data):
        # CREATE 요청에 대해 create 메서드를 오버라이딩, 유저 생성 &토큰 생성.
...
class LoginSerializer(serializers.Serializer):
    ...
    # write_only=True 옵션을 통해 클라이언트->서버의 역직렬화는 가능하지만, 서버->클라이언트 방향의 직렬화는 불가능하도록.
